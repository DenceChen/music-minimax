...
- Name: ux.spec.js >> Music MiniMax 用户体验测试 >> 按钮状态测试 >> 输入框为空时发送按钮仍然可用
- Location: tests/ux.spec.js:33:5
...
Locator:  locator('button:has-text("发送")')
Expected: enabled
Received: disabled
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("发送")')
    9 × locator resolved to <button disabled class="send-btn">发送</button>
...
> 36  |       await expect(sendButton).toBeEnabled();
  ...
  120 |   });
  121 | 
  122 |   test.describe('响应式设计测试', () => {
  123 |     test('桌面端 - 全功能布局', async ({ page }) => {
  124 |       await page.setViewportSize({ width: 1280, height: 720 });
  125 | 
  126 |       await expect(page.locator('h1')).toBeVisible();
  127 |       await expect(page.locator('input[placeholder*="描述"]')).toBeVisible();
  128 |       await expect(page.locator('button:has-text("发送")')).toBeVisible();
  129 |     });
  130 | 
  131 |     test('平板端 - 纵向布局', async ({ page }) => {
  132 |       await page.setViewportSize({ width: 768, height: 1024 });
  133 | 
  134 |       await expect(page.locator('h1')).toBeVisible();
  135 |       await expect(page.locator('input[placeholder*="描述"]')).toBeVisible();
  136 |     });
```